...
| **Every deploy is recorded, and the record can be checked** | Every step of a release as its own record, each chained to the one before, with the commit, artefact hash, approver, backup, who deployed and from where. `make audit-verify` re-checks every link | `bin/audit-log`, `tasks/audit/`, [the field reference](ledger.md) | `tests/test_audit_log.py`: edits, deletions, reordering and a re-hashed forgery are all found; concurrent writers keep one chain |
